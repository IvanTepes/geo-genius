...
function checkAnswer(userAnswer) {
    ...
    const categoryValidation = {
        flags: (userAnswer, correctCountry) =>
            userAnswer === correctCountry.name,
        population: (userAnswer, correctCountry) =>
            userAnswer ===
            new Intl.NumberFormat()
                .format(correctCountry.population)
                .toString(),
        capital: (userAnswer, correctCountry) =>
            userAnswer === correctCountry.capital,
    };

    const validateAnswer = categoryValidation[selectedGameCategory];

    if (validateAnswer(userAnswer, correctCountry)) {
...
function setAnswerButtonsText(answers) {
    ...
    for (let i = 0; i < gameAnswersBtn.length; i++) {
        ...
        if (selectedGameCategory === 'flags') {
            answerText = answers[i].name;
        } else if (selectedGameCategory === 'population') {
            answerText = new Intl.NumberFormat()
                .format(answers[i].population)
                .toString();
        } else {
            answerText = answers[i].capital;
        }
